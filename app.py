...
with col1:
    ...
    if ipk <= 2.5:
        mu_layak = 0.0
        rumus_layak = f"Because IPK ({ipk}) \\le 2.5, maka \\mu = 0.0"
    elif 2.5 < ipk < 3.5:
        mu_layak = (ipk - 2.5) / (3.5 - 2.5)
        rumus_layak = f"\\mu = \\frac{{{ipk} - 2.5}}{{3.5 - 2.5}} = \\frac{{{(ipk - 2.5):.2f}}}{{1.0}} = {mu_layak:.2f}"
    else:
        mu_layak = 1.0
        rumus_layak = f"Karena IPK ({ipk}) \\ge 3.5, maka \\mu = 1.0"
    
    st.header("📋 [2] Tabel Nilai Hasil Fuzzifikasi")
    st.write("Ringkasan perolehan nilai derajat keanggotaan (Fuzzy Output) untuk IPK yang diinput:")
    
    # Data Tabel Keanggotaan
    data_tabel = {
        "Himpunan Fuzzy": ["📉 Tidak Layak", "📉 Dipertimbangkan", "📈 Layak"],
        "Formula / Kurva": ["Bahu Turun (Linier)", "Segitiga (Triangular Mf)", "Bahu Naik (Linier)"],
        "Derajat Keanggotaan \u03bc(x)": [f"{mu_tidak_layak:.4f}", f"{mu_dipertimbangkan:.4f}", f"{mu_layak:.4f}"]
    }
    st.table(data_tabel)

    st.markdown("---")
    
    st.header("🧮 [3] Perhitungan Detail Jalannya Rumus")
    st.write("Kalkulasi breakdown matematika *fuzzifikasi* secara *real-time* berdasarkan nilai input:")
    ...
    # --- INTERPRETASI HASIL ---
    st.header("📝 [4] Interpretasi Hasil Akhir")
...
# ==========================================
with col2:
    st.header("📊 [5] Grafik Himpunan Fuzzy")
    ...
    # Plot titik pertemuan interaktif di semua kurva yang bersinggungan
    ax.scatter([ipk, ipk, ipk], [mu_tidak_layak, mu_dipertimbangkan, mu_layak], color='purple', s=45, zorder=5)
    ...
    st.header("📐 [6] Dokumentasi Fungsi Keanggotaan")
    ...
    tab1, tab2, tab3 = st.tabs(["📉 Tidak Layak", "📉 Dipertimbangkan", "📈 Layak"])
    ...
    with tab2:
        ...
        \frac{x - 1.5}{2.5 - 1.5}, & 1.5 < x \le 2.5 \\ 
        \frac{3.5 - x}{3.5 - 2.5}, & 2.5 < x < 3.5 
        \end{cases}
        """)
    with tab3:
